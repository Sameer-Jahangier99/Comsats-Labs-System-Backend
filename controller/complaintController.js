...
const registerComplaint = asyncHandler(async (req, res) => {
    ...
    const { user, product, title, lab, note, type } = req.body;
    const registerdComplaint = await Complaint.create({
        user, product, title, lab, note, type
    })
    if (registerdComplaint) {
        res.json(registerdComplaint)
    } else {
        res.status(400);
...
const approvedByDco = asyncHandler(async (req, res) => {
    ...
    if (complaint) {
        await Complaint.findOneAndUpdate({ _id: req.body._id }, { $set: { dcoApproved: true } })
...
const rejectedByDco = asyncHandler(async (req, res) => {
    const complaint = await Complaint.findById(req.body._id);
    if (complaint) {
        await Complaint.findOneAndUpdate({ _id: req.body._id }, { $set: { dcoApproved: false } })
...
const acceptedSoftwareComplaint = asyncHandler(async (req, res) => {
    const complaint = await Complaint.find({ dcoApproved: true, committeApproved: true, nocApproved: true }).populate("product user", "name lab ")
...
const acceptedhardwareComplaint = asyncHandler(async (req, res) => {
    const complaint = await Complaint.find({ dcoApproved: true, committeApproved: true, worksApproved: true }).populate("product user", "name lab ")
...
const allhardwareComplaint = asyncHandler(async (req, res) => {
    const complaint = await Complaint.find({ type: "hardware" }).populate("product user", "name lab ")
...
const allSoftwareComplaint = asyncHandler(async (req, res) => {
    const complaint = await Complaint.find({ type: "software" }).populate("product user", "name lab ")
...
const approveByCommittee = asyncHandler(async (req, res) => {
    const complaint = await Complaint.findById(req.body._id);
    if (complaint) {
        await Complaint.findOneAndUpdate({ _id: req.body._id }, { $set: { committeApproved: true } })
        res.json("Complaint has been approved")
    } else {
        res.status(404);
        throw new Error("Complaint not found");
...
const rejectByCommittee = asyncHandler(async (req, res) => {
    const complaint = await Complaint.findById(req.body._id);
    if (complaint) {
        await Complaint.findOneAndUpdate({ _id: req.body._id }, { $set: { committeApproved: false } })
        res.json("Complaint has been rejected")
    } else {
        res.status(404);
        throw new Error("Complaint not found");
...
const allComplaintAprrovedByCommittee = asyncHandler(async (req, res) => {
    const complaints = await Complaint.find({
        committeApproved: true, dcoApproved: true, type: "software"
    }).populate(
        'product user',
        'name lab ',
    );
...
const allHardwareComplaintAprrovedByCommittee = asyncHandler(async (req, res) => {
    const complaints = await Complaint.find({
        committeApproved: true, dcoApproved: true, type: "hardware"
    }).populate(
        'product user',
        'name lab ',
    );
...
const approveByWorks = asyncHandler(async (req, res) => {
    const complaint = await Complaint.findById(req.body._id);
    if (complaint) {
        await Complaint.findOneAndUpdate({ _id: req.body._id }, { $set: { worksApproved: true } })
        res.json("Complaint has been approved")
    } else {
        res.status(404);
        throw new Error("Complaint not found");
...
const rejectByWorks = asyncHandler(async (req, res) => {
    const complaint = await Complaint.findById(req.body._id);
    if (complaint) {
        await Complaint.findOneAndUpdate({ _id: req.body._id }, { $set: { worksApproved: false } })
        res.json("Complaint has been rejected")
    } else {
        res.status(404);
        throw new Error("Complaint not found");
...
const approveByNoc = asyncHandler(async (req, res) => {
    const complaint = await Complaint.findById(req.body._id);
    if (complaint) {
        await Complaint.findOneAndUpdate({ _id: req.body._id }, { $set: { nocApproved: true } })
        res.json("Complaint has been approved")
    } else {
        res.status(404);
        throw new Error("Complaint not found");
...
const rejectByNoc = asyncHandler(async (req, res) => {
    const complaint = await Complaint.findById(req.body._id);
    if (complaint) {
        await Complaint.findOneAndUpdate({ _id: req.body._id }, { $set: { nocApproved: false } })
        res.json("Complaint has been rejected")
    } else {
        res.status(404);
        throw new Error("Complaint not found");
...
const complaintCompleted = asyncHandler(async (req, res) => {
    ...
    if (complaint) {
        await Complaint.updateOne({ _id: req.params._id }, { $set: { status: "completed" } })
...
const allComplaintAprrovedByNOC = asyncHandler(async (req, res) => {
    const complaints = await Complaint.find({ nocApproved: true, type: "software" }).populate(
        'product user',
        'name lab ',
    );
    if (complaints && complaints.length > 0) {
...
const allComplaintAprrovedByWorks = asyncHandler(async (req, res) => {
    const complaints = await Complaint.find({ worksApproved: true, type: "hardware" }).populate(
        'product user',
        'name lab ',
    );
    if (complaints && complaints.length > 0) {
...
const complaintById = asyncHandler(async (req, res) => {
    ...
    const complaint = await Complaint.findById(_id).populate('user product', '-password');
    if (complaint) {
        res.json(complaint)
    } else {
        res.status(404);
